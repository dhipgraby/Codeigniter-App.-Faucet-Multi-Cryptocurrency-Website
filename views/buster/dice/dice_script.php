...
var coin = $('[name="coin"]:checked').attr('id');  
 ...
 var balance = $('#' + coin + '_bal').html() * 100000000;

if(coin == 'doge'){

var balance = parseInt($('#doge_bal').html().replace(/,/g, ''));
...
url: "<?php echo base_url(); ?>dice/check_bet/"+ amount + "/" + multi +"/" + roll + '/'+ coin,
type: "POST",
data: { },
...
roll = (n % 2 == 1) ? 1 : 2;
  ...
  var amount = parseFloat(document.getElementById('base_bet').value);
  var multi = document.getElementById('multiplier').value;
  var profit2 =  parseFloat(document.getElementById('reward_profit').value);
...
var c_bal = $('#'+coin+'_bal').text();
$('#c_balance').val(c_bal + ' ' + coin);
...
function update_table() {
  ...
  xmlhttp.open("GET", "<?php echo base_url(); ?>dice/result_table", true);
  xmlhttp.send();
} 
...
function update_autobet() {
  ...
  xmlhttp.open("GET", "<?php echo base_url(); ?>dice/autobet_result", true);
  xmlhttp.send();
} 
...
$('#bet2').val(parseFloat(document.getElementById('base_bet').value));
